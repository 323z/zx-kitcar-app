...
document.addEventListener('keyup', function(e){ keys[e.key] = false; e.preventDefault(); });
...
var BTN = {
  ...
  CAMERA:  {x:134, y:220, w:24, h:22, label:'CAM'},
  START:   {x:100, y:195, w:30, h:18, label:'START'},
  RESET:   {x:134, y:195, w:24, h:18, label:'RST'}
};
...
function getTouchPos(t) {
  ...
  return {
    x: (t.clientX - rect.left) * 256 / rect.width,
    y: (t.clientY - rect.top)  * 256 / rect.height
  };
}

function scanTouches(e) {
  touchState = {};
  for (var i=0;i<e.touches.length;i++){
    var p = getTouchPos(e.touches[i]);
    for (var k in BTN) if (pointInBtn(p.x,p.y,BTN[k])) touchState[k] = true;
  }
}

canvas.addEventListener('touchstart', function(e){ e.preventDefault(); scanTouches(e); }, {passive:false});
canvas.addEventListener('touchmove',  function(e){ e.preventDefault(); scanTouches(e); }, {passive:false});
canvas.addEventListener('touchend',   function(e){ e.preventDefault(); scanTouches(e); }, {passive:false});
canvas.addEventListener('touchcancel',function(e){ e.preventDefault(); touchState = {}; }, {passive:false});
...
// ---------- Car State ----------
var car = {
  x: 128, y: 100, angle: -Math.PI/2, speed: 0, rpm: 1000,
...
function playEngineSound() {
  if (!audioCtx || car.engineSound) return;
...
function updateEngineSound() {
  ...
  car.engineSound.osc.frequency.value = 60 + car.rpm * 0.3;
  car.engineSound.gain.gain.value = 0.02 + (isDown('THROTTLE')?0.06:0) + (car.boost>0?0.04:0);
}
function stopEngineSound() {
  if (car.engineSound) { try{car.engineSound.osc.stop();}catch(e){} car.engineSound = null; }
}
...
function update(dt) {
  ...
  if (car.accelStart > 0) {
    ...
    if (elapsed > 20) car.accelStart = -1;
  }
  ...
  if (isDown('RESET')) {
    ...
    car.smoke=[]; car.racing=false; car.accelStart=0; car.bestLap=Infinity;
    stopEngineSound();
  }
...
function drawHUD() {
  ...
  ctx.fillRect(100,4,60*Math.max(0,Math.min(1,rpmPct)),8);
  ...
  var boostPct = car.boost>0?car.boost/8:(car.boostCooldown>0?1-car.boostCooldown/25:1);
  ctx.fillRect(100,14,60*boostPct,6);
  ...
  if (!car.racing) {
    ...
    ctx.fillText('READY',128,105);
    ctx.font='9px monospace'; ctx.fillStyle='#fff';
    ctx.fillText('Throttle+Brake=Start',128,120);
    ctx.fillText('Push-Start if stall',128,132);
    ctx.textAlign='left';
  }
